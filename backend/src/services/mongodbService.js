...
class MongoDBService {
  ...
  async getDashboardStats() {
    try {
      ...
      const activeMining = await this.count('MiningSession', { status: 'active' });
  ...
  async getUserByIdAdmin(userId) {
    try {
      ...
      const miningSessionsCount = await this.count('MiningSession', { userId });
  ...
  async getMiningSessions({ page = 1, limit = 20, status, userId }) {
    try {
      ...
      const result = await this.findMany('MiningSession', filters, {
        page,
        limit,
        orderBy: { column: 'createdAt', ascending: false },
        populate: 'userId'
